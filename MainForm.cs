...
public sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        var optionsFlow = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true, Padding = new Padding(4) };

        optionsFlow.Controls.Add(new Label { Text = "Output folder:", AutoSize = true, Anchor = AnchorStyles.Left, Margin = new Padding(3, 8, 3, 3) });
        _outputFolder = new TextBox { Width = 320, PlaceholderText = "(same folder as each input file)" };
        optionsFlow.Controls.Add(_outputFolder);
        optionsFlow.Controls.Add(MakeButton("Browse…", (_, _) => BrowseOutputFolder()));

        _ocrEnabled = new CheckBox { Text = "OCR scanned pages", Checked = true, AutoSize = true, Margin = new Padding(12, 8, 3, 3) };
        _forceOcr = new CheckBox { Text = "Force OCR on all PDF pages", AutoSize = true, Margin = new Padding(12, 8, 3, 3) };
        optionsFlow.Controls.Add(_ocrEnabled);
        optionsFlow.Controls.Add(_forceOcr);

        optionsFlow.Controls.Add(new Label { Text = "OCR language:", AutoSize = true, Margin = new Padding(12, 8, 3, 3) });
        _ocrLang = new TextBox { Width = 80, Text = "eng" };
        optionsFlow.Controls.Add(_ocrLang);
    ...
    {
        using var dialog = new FolderBrowserDialog { Description = "Add all PDF/DOCX files under a folder" };
        if (dialog.ShowDialog(this) == DialogResult.OK)
    ...
    {
        using var dialog = new FolderBrowserDialog { Description = "Folder for the converted .md files" };
        if (dialog.ShowDialog(this) == DialogResult.OK)
